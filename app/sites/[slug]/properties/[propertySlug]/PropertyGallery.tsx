  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        {sorted.length === 1 ? (
          <div
            className="rounded-2xl overflow-hidden cursor-pointer aspect-[16/7] bg-gray-100"
          ...
          <div className="grid grid-cols-4 gap-2 rounded-2xl overflow-hidden max-h-[500px]">
            {/* Principal */}
            <div
              className="col-span-4 sm:col-span-2 row-span-2 cursor-pointer bg-gray-100"
              onClick={() => openLightbox(0)}
            >
            ...
            {sorted.slice(1, 5).map((img, i) => (
              <div
                key={i}
                className="hidden sm:block cursor-pointer relative bg-gray-100"
                onClick={() => openLightbox(i + 1)}
              >
                <img src={img.url} alt="" className="w-full h-full object-cover hover:opacity-90 transition-opacity" />
                {i === 3 && sorted.length > 5 && (
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                    <span className="text-white font-bold text-lg">+{sorted.length - 5}</span>
                  </div>
                )}
              </div>
            ))}
        ...
        {sorted.length > 1 && (
          <button
            onClick={() => openLightbox(0)}
            className="mt-2 text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors"
          >
